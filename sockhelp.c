...
// Abrir porta para driver Listener
int open_sock(char *porta)
{
  int length;
  u_short LocalPort;
  int msgsock;
  ...
  sock = socket(AF_INET, SOCK_STREAM, 0);
  if (sock < 0) {
    logMsg("tcpip: Abrindo socket tipo stream\n");
    exitdriver(1);
  }

  /* Nomeia o socket */
  LocalPort = atoi(porta);
  server.sin_family = AF_INET;
  server.sin_addr.s_addr = INADDR_ANY;
  server.sin_port = htons(LocalPort);
  if (bind(sock, (struct sockaddr *)&server, sizeof server) < 0) {
    logMsg("tcpip: Associando socket tipo stream\n");
    exitdriver(1);
  }

  /* Descobre a porta associada e imprime */
  length = sizeof(server);
  if (getsockname(sock, (struct sockaddr *)&server, &length) < 0) {
    logMsg("tcpip: Obtendo nome do socket");
    exitdriver(1);
  }
  logMsg("Porta do socket: #%d\n", ntohs(server.sin_port));

  /* Comeca a aceitar conexoes */
  listen(sock, 5);
  msgsock = accept(sock, (struct sockaddr *)0, (int *)0);

    /* Depois de aceitar a conexao, le um primeiro caracter para
    ** saber se esta' apenas recebendo uma mensagem de controle ou
    ** um arquivo. Se for um arquivo, le um bloco de dados
    ** que contem o nome do arquivo destino terminado por NULL e
    ** que tambem ja contem o inicio deste arquivo
    */
  if (msgsock == -1)
      logMsg("tcpip: Accept failed\n");
  return msgsock;
}
