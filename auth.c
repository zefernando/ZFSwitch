...
int prepareResponse(char * resp)
{
	...
	msg->msgtype += 10;
        ...
        */
        if (authP->tipoParc != '0') 
        {
          memcpy(msg->aval_balance, authP->valorParc,12);
  	  msg->aval_balance[12] = '\0';
	  debug("Valor Parcela(aval_balance) %s\n", msg->aval_balance);
	}
        
        if (atoi(authP->dispcompras) > 0) 
        {
          memcpy(msg->aval_balance, authP->dispcompras,12);
  	  debug("Disponivel compras%s\n", msg->aval_balance);
	}
